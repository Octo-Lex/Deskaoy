...
class TestInMemorySpanExporterFixture:
    """Fixture wrapper for InMemorySpanExporter to handle dependency issues."""

    def test_create_exporter(self):
        """Test that InMemorySpanExporter can be imported and instantiated."""
        try:
...
class TestRedactingExporterFixture:
    """Fixture wrapper for RedactingExporter to handle dependency issues."""

    def test_create_exporter(self):
        """Test that RedactingExporter can be instantiated."""
        try:
            from opentelemetry.sdk.trace.export import InMemorySpanExporter
            from deskaoy.tracing.exporters import RedactingExporter
            inner = InMemorySpanExporter()
            exporter = RedactingExporter(inner)
            assert exporter is not None
        except ImportError:
            self.skipTest("opentelemetry-sdk not installed")
